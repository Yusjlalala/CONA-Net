...
class SpatialAttentionBlock3d(nn.Module):
    def __init__(self, in_channels):
        ...
        self.gamma = nn.Parameter(torch.Tensor([1.]))  # 改成1了，原来是torch.zeros(1)
    ...
    def forward(self, x):
        ...
        """
        N, C, H, W, D = x.size()
        # compress x: [N,C,H,W,Z]-->[N,H*W*Z,C], make a matrix transpose
        proj_query = self.query(x).view(N, -1, W * H * D).permute(0, 2, 1)  # -> [N,W*H*D,C]
        proj_key = self.key(x).view(N, -1, W * H * D)  # -> [N,C,H*W*D]
        proj_judge = self.judge(x).view(N, -1, W * H * D).permute(0, 2, 1)  # -> [N,W*H*D,C]
        ...
        proj_value = self.value(x).view(N, -1, H * W * D)  # -> C*N
        weights = torch.matmul(proj_value, affinity)
        weights = weights.view(N, C, H, W, D)
...
class ChannelAttentionBlock3d(nn.Module):
    ...
    def forward(self, x):
        """
        :param x: input( NxCxHxWxD )
        :return: affinity value + x
        """
        N, C, H, W, D = x.size()
        proj_query = x.view(N, C, -1).permute(0, 2, 1)
        proj_key = x.view(N, C, -1)
        proj_judge = x.view(N, C, -1).permute(0, 2, 1)
...
class CS2Net(nn.Module):
    def __init__(self, input_channels, output_channels):
        """
        :param output_channels: the object classes number.
        :param input_channels: the channels of the input image.
        """
        super(CS2Net, self).__init__()
        self.enc_input = ResEncoder3d(input_channels, 16)
        ...
        self.final = nn.Conv3d(16, output_channels, kernel_size=1)
