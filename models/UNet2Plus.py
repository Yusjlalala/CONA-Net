...
class UNet2Plus(nn.Module):
    def __init__(self, input_channels, output_channels, feature_scale=4, is_ds=True):
        ...
        nb_filter = [int(x / self.feature_scale) for x in nb_filter]
        ...
        if self.is_ds:
            self.final1 = nn.Conv3d(nb_filter[0], output_channels, kernel_size=1)
            self.final2 = nn.Conv3d(nb_filter[0], output_channels, kernel_size=1)
            self.final3 = nn.Conv3d(nb_filter[0], output_channels, kernel_size=1)
            self.final4 = nn.Conv3d(nb_filter[0], output_channels, kernel_size=1)
        else:
